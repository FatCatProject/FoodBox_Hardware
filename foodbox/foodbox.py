...
class FoodBox:
    ...
    def sync_with_brainbox(self):
        ...
        # success previously was false
        success = True  # type: bool
        # TODO - Sync with brainbox

        for feeding_log in uid_to_sync:
            respone = requests.post("http://" + self.__brainbox_ip_address + ":" + self.__brainbox_port_number+"/syncfeedinglogs",
                                    json={"feedingid":feeding_log.get_id(),"cardid":feeding_log.get_card(),"opentime":feeding_log.get_open_time(),
                                          "closetime":feeding_log.get_close_time(),"startweight":feeding_log.get_start_weight(),
                                                "endweight":feeding_log.get_end_weight(),"foodboxid":self.__foodbox_id})
            if(respone>200 && respone<300): # or 200
                self.mark_feeding_logs_synced(feeding_log)
                print("Data sent:"+self.__foodbox_id+feeding_log)
            else:
                print("Error"+self.__foodbox_id+"wasnt recieved from"+ self.__foodbox_id)
                success=False
